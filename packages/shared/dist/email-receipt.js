...
import { formatDurationHms, formatReceiptDate, } from './format.js';
export function renderEmailReceipt(totals, locale, manageUrl, deleteUrl) {
    ...
    const lines = Object.keys(totals.platforms)
    ...
    const htmlRows = lines
        .map((l) => `<tr><td style="padding:4px 0;font-family:monospace">${l.label}</td><td style="text-align:right;font-family:monospace">${l.qty}</td><td style="text-align:right;font-family:monospace">${l.time}</td></tr>`)
        .join('');
